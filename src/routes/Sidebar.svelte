...
<div class="absolute z-50 top-0 right-0">
	<div class="w-full flex justify-end p-4">
		...
		>
			<svg class="w-8 text-orange-500 h-8" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"
	...
	{#if shown}
		<button
			in:fly={{ x: -2000 }}
			out:fly={{ x: -2000 }}
			class="fixed transition top-0 left-0 z-30 w-full h-full cursor-default bg-black/10"
			on:click={() => (shown = !shown)}
		></button>
		<div
			in:fly={{ x: -200 }}
			out:fly={{ x: -200 }}
			class="fixed top-0 left-0 z-40 w-64 h-screen p-4 overflow-y-auto bg-white dark:bg-darkmode-500"
			tabindex="-1"
		...
		>
			...
			<button
				...
				aria-controls="drawer-navigation"
				class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded text-sm p-1.5 absolute top-2.5 end-2.5 inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white"
			>
				<svg
					aria-hidden="true"
					class="w-5 h-5"
			...
			<div class="py-4 overflow-y-auto">
				<ul class="space-y-2 font-medium">
					...
					<li>
						...
						>
							<svg
								class="flex-shrink w-5 h-5 text-gray-500 transition duration-75 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-white"
								aria-hidden="true"
								xmlns="http://www.w3.org/2000/svg"
								viewBox="0 0 24 24"
							>
          ...
          {/if}
					<li>
						<button
							...
							class="flex justify-center w-full items-center p-2 text-gray-900 rounded dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700 group"
						>
							<svg
								class="flex-shrink-0 w-5 h-5 text-gray-500 transition duration-75 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-white"
								aria-hidden="true"
