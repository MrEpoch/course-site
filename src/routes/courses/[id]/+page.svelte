...
{#if form?.error}
	<h1 class="text-xl text-red-500 font-bold leading-tight tracking-tight md:text-2xl">
		{form?.error}
	</h1>
{/if}
			...
			<div class="w-full flex flex-col gap-4 h-full">
				...
				{#if session}
					<form class="w-full" method="POST">
						<button
							class="text-white bg-orange-600 hover:bg-orange-700 font-medium rounded text-md px-5 py-2.5 text-center dark:bg-orange-600 dark:hover:bg-orange-700 transition"
							>Enroll now</button
						>
