...
<div class="min-h-screen dark:bg-darkmode-500 w-full flex py-12">
	<div class="max-w-screen-xl mx-auto h-full py-8 px-6 w-full flex flex-1 justify-between items-center flex-col">
		<div class="flex items-center justify-center w-full h-full py-8 flex-wrap">
			<a
				href="/courses?category=all"
				data-sveltekit-reload
				class={category === 'all' || !category ? style.styled : style.normal}>All Courses</a
			>
			<a
				href="/courses?category=html"
				data-sveltekit-reload
				class={category === 'html' ? style.styled : style.normal}>HTML</a
			>
			<a
				href="/courses?category=css"
				data-sveltekit-reload
				class={category === 'css' ? style.styled : style.normal}>CSS</a
			>
			<a
				href="/courses?category=react"
				data-sveltekit-reload
				class={category === 'react' ? style.styled : style.normal}>React</a
			>
			<a
				href="/courses?category=svelte"
				data-sveltekit-reload
				class={category === 'svelte' ? style.styled : style.normal}>SvelteKit</a
			>
		</div>
		<div class="flex flex-1 flex-wrap gap-6 justify-center">
